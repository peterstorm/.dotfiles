...
```json
{
  ...
  "plan_file": ".opencode/plans/YYYY-MM-DD-slug.md",
  "spec_file": ".opencode/specs/YYYY-MM-DD-slug/spec.md",
  "github_issue": 42,
  ...
  "phase_artifacts": {
    ...
    "specify": ".opencode/specs/YYYY-MM-DD-slug/spec.md",
    "clarify": "skipped",
    "architecture": ".opencode/plans/YYYY-MM-DD-slug.md"
  },
...
### Per-Task Fields Set by Plugin Hooks

These fields are set **automatically** by plugin hooks. They cannot be set manually (guard hook blocks state file writes via Bash).

| Field | Set By | When |
|-------|--------|------|
| `start_sha` | `validate-task-execution` hook | tool.execute.before: HEAD SHA stored before task agent spawns |
| `status` → "implemented" | `update-task-status` hook | session.idle after implementation agent completes |
| `tests_passed` | `update-task-status` hook | Extracted from agent transcript (Maven/Node/Vitest/pytest markers) |
| `test_evidence` | `update-task-status` hook | Description of which markers were found |
| `files_modified` | `update-task-status` hook | Files touched by agent (from Write/Edit tool calls in transcript) |
| `new_tests_written` | `verify-new-tests` hook | Git diff scanned for new test method patterns (scoped to files_modified) |
| `new_test_evidence` | `verify-new-tests` hook | Count and details of new test methods found |
| `review_status` | `store-review-findings` hook | session.idle after review agent completes |
| `critical_findings` | `store-review-findings` hook | Parsed from reviewer output |
| `advisory_findings` | `store-review-findings` hook | Parsed from reviewer output |
...
### Wave Gate Checks (by `complete-wave-gate` hook)
