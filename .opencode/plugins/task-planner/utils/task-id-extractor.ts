 ...
 */
const VERB_TASK_PATTERN =
  /(?:implement|fix|complete|execute|run|start|do|work(?:ing)?\s+on)\s+(T\d+)/i;
 ...
 */
export function isCanonicalFormat(text: string): boolean {
 ...
 */
export function getCanonicalFormat(taskId: string): string {
 ...
 */
export function validateTaskIdFormat(
  text: string
): "canonical" | "valid" | "none" {
