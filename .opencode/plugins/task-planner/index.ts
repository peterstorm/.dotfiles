...
export const TaskPlannerPlugin: Plugin = async ({
  ...
  return {
    ...
    "tool.execute.before": async (
      ...
      const legacyInput: ToolExecuteInput = {
        tool: input.tool,
        args: output.args,
      };
      ...
      if (input.tool.toLowerCase() === "task") {
        const taskInput: TaskToolInput = {
          tool: input.tool,
          args: output.args as TaskToolInput["args"],
        };
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "file.edited") {
        const fileEvent = event as unknown as { type: "file.edited"; properties: { file: string } };
        const path = fileEvent.properties?.file;
        
        if (!stateManager || !path) return;
