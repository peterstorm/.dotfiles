...
export const ARTIFACT_PHASES: readonly Phase[] = [
  "brainstorm",
  "specify",
  "clarify",
  "architecture",
...
export const ARTIFACT_PATH_PATTERN =
  /(?:saved|created|wrote|generated)\s+(?:to\s+)?['"]?([^\s'"]+\.md)['"]?/i;
...
export const PROTECTED_STATE_PATTERNS: readonly RegExp[] = [
  /\.opencode\/state\/.*\.json$/,
  /\.opencode\/specs\/.*\.md$/,
  /\.opencode\/plans\/.*\.md$/,
...
export const BASH_WRITE_PATTERNS: readonly RegExp[] = [
  /\becho\b.*>/,
  /\bprintf\b.*>/,
  /\bcat\b.*>/,
  /\btee\b/,
...
export const IMPLEMENTATION_AGENTS: readonly AgentType[] = [
...
export const REVIEW_AGENTS: readonly AgentType[] = [
  "reviewer-agent",
  "spec-check-agent",
] as const;
...
export const NEW_TEST_METHOD_PATTERNS: readonly RegExp[] = [
...
export const TEST_EVIDENCE_PATTERNS = {
  ...
  maven: {
    ...
    results:
      /Tests run:\s*\*{0,2}(\d+)\*{0,2},\s*Failures:\s*\*{0,2}0\*{0,2},\s*Errors:\s*\*{0,2}0\*{0,2}/,
  },
 ...
 */
export const TASK_EXECUTION_AGENTS: readonly string[] = [
  "code-implementer-agent",
  "java-test-agent",
  "ts-test-agent",
  "frontend-agent",
 ...
 */
export const CRASH_DETECTION_AGENTS: readonly string[] = [
  "code-implementer-agent",
  "java-test-agent",
  "ts-test-agent",
  "frontend-agent",
 ...
 */
export const TASK_REVIEW_AGENTS: readonly string[] = [
  "reviewer-agent",
  "spec-check-agent",
] as const;
 ...
 */
export const COMPLETED_TASK_STATUSES: readonly string[] = [
  "implemented",
  "completed",
] as const;
...
This ensures proper phase sequencing and review gates.`,
  ...
  STATE_FILE_WRITE_BLOCKED: (file: string) =>
    `BLOCKED: Cannot write to state file "${file}" via Bash.

State files must be modified through the task-planner plugin's state manager
to ensure proper locking and consistency.`,

  PHASE_ORDER_VIOLATION: (current: Phase, attempted: Phase) =>
    `BLOCKED: Cannot skip to phase "${attempted}" from "${current}".
...
Complete the current phase before proceeding.`,
  ...
  TASK_DEPENDENCY_VIOLATION: (taskId: string, deps: string[]) =>
    `BLOCKED: Cannot execute task ${taskId} - dependencies not met.

The following tasks must be completed first:
  ${deps.join(", ")}`,

  WAVE_ORDER_VIOLATION: (taskWave: number, currentWave: number) =>
    `BLOCKED: Cannot execute task from wave ${taskWave} while still in wave ${currentWave}.
...
Consider migrating to .opencode/ directory structure.`,
  ...
  // Phase 3 errors
  REVIEW_GATE_NOT_PASSED: (prevWave: number) =>
    `BLOCKED: Wave ${prevWave} review gate not passed.

Run /wave-gate to complete tests, reviews, and advance to the next wave.`,

  REVIEW_GATE_BLOCKED: (prevWave: number, reasons: string[]) =>
    `BLOCKED: Wave ${prevWave} is BLOCKED due to:
${reasons.map((r) => `  - ${r}`).join("\n")}

Fix the issues and re-run /wave-gate.`,

  WAVE_GATE_TESTS_MISSING: (wave: number, missing: string[]) =>
    `FAILED: Not all wave ${wave} tasks have test evidence.
...
Re-spawn agents that failed to run tests.`,
  ...
  WAVE_GATE_NEW_TESTS_MISSING: (wave: number, missing: string[]) =>
    `FAILED: Not all wave ${wave} tasks satisfied new-test requirement.
...
Re-spawn agents that failed to write tests.`,
  ...
  WAVE_GATE_REVIEWS_MISSING: (wave: number, unreviewed: string[]) =>
    `FAILED: Not all wave ${wave} tasks have been reviewed.

Unreviewed: ${unreviewed.join(", ")}

Spawn review-invoker agents for unreviewed tasks.`,

  WAVE_GATE_CRITICAL_FINDINGS: (wave: number, count: number) =>
    `FAILED: Wave ${wave} code review has ${count} critical findings.
