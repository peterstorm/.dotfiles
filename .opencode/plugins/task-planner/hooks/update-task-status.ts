...
export async function updateTaskStatus(
  ...
  if (task.status === "implemented" && task.tests_passed !== undefined) {
    console.log(
      `[task-planner] Task ${taskId} already implemented with test evidence`
    );
    return null;
  }
  ...
  const waveComplete = await checkWaveComplete(
    taskGraph.current_wave,
    stateManager
  );

  // Get remaining tasks
  const remainingTasks = getRemainingTaskIds(
  ...
  if (remainingTasks.length > 0) {
    console.log(
      `Remaining in wave ${taskGraph.current_wave}: ${remainingTasks.join(", ")}`
    );
  }
 ...
 */
export function extractTestEvidence(transcript: string): TestEvidence {
  ...
  const vitestPassed = cleanTranscript.match(
    TEST_EVIDENCE_PATTERNS.vitest.passed
  );
  const vitestFailed = cleanTranscript.match(
    TEST_EVIDENCE_PATTERNS.vitest.failed
  );
  ...
  // Try Mocha (Node)
  const mochaPassing = cleanTranscript.match(
    TEST_EVIDENCE_PATTERNS.mocha.passing
  );
  const mochaFailing = cleanTranscript.match(
    TEST_EVIDENCE_PATTERNS.mocha.failing
  );
  ...
  // Try pytest
  const pytestPassed = cleanTranscript.match(
    TEST_EVIDENCE_PATTERNS.pytest.passed
  );
  const pytestFailed = cleanTranscript.match(
    TEST_EVIDENCE_PATTERNS.pytest.failed
  );
 ...
 */
export async function checkWaveComplete(
  wave: number,
  stateManager: StateManager
): Promise<boolean> {
 ...
 */
export function getRemainingTaskIds(
  taskGraph: TaskGraph,
  wave: number
): string[] {
 ...
 */
export function parseFilesModified(transcript: string): string[] {
