...
function createSkillInput(skillName: string): ToolExecuteInput {
  return {
    tool: "skill",
    args: { name: skillName },
  };
...
function createNonSkillInput(tool: string): ToolExecuteInput {
  return {
    tool,
    args: { path: "/some/path" },
  };
  ...
  describe("when task graph exists", () => {
    ...
    describe("valid transitions", () => {
      ...
      it("allows specify → architecture (when clarify skipped)", () => {
        expect(
          isValidTransition("specify", "architecture", ["clarify"])
        ).toBe(true);
      });
