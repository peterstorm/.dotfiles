 ...
 */
export function validateTaskExecution(input, taskGraph) {
    ...
    // All checks passed
    console.log(`[task-planner] Task execution validated: ${taskId} (wave ${task.wave})`);
    return {
        allowed: true,
        taskId,
    };
 ...
 */
export function checkWaveOrder(task, currentWave) {
 ...
 */
export function checkReviewGate(prevWave, taskGraph) {
    const gate = taskGraph.wave_gates[prevWave];
 ...
 */
export function isTaskReady(task, taskGraph) {
    const currentWave = taskGraph.current_wave;
    // Wave order check
    if (task.wave > currentWave) {
        return false;
 ...
 */
export function getReadyTasks(taskGraph) {
 ...
 */
export function getRemainingTasks(taskGraph) {
    return taskGraph.tasks.filter((task) => task.wave === taskGraph.current_wave &&
        !COMPLETED_TASK_STATUSES.includes(task.status));
}
//# sourceMappingURL=validate-task-execution.js.map