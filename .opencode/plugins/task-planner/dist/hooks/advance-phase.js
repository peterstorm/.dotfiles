...
import { PHASE_COMPLETION_PATTERNS, ARTIFACT_PATH_PATTERN, CLARIFICATION_MARKER_PATTERN, CLARIFY_MARKER_THRESHOLD, PHASE_ORDER, } from "../constants.js";
// ============================================================================
 ...
 */
export async function advancePhase(content, taskGraph, stateManager, projectDir) {
    ...
    if (completedPhase === "specify" && nextPhase === "clarify") {
        ...
        console.log(`[task-planner] Clarification markers in spec: ${markerCount} (threshold: ${CLARIFY_MARKER_THRESHOLD})`);
 ...
 */
export function detectCompletedPhase(content, currentPhase) {
 ...
 */
export function extractArtifactPath(content) {
 ...
 */
export function resolveArtifactPath(artifact, projectDir) {
 ...
 */
export async function countClarificationMarkers(specPath) {
    if (!specPath || !existsSync(specPath)) {
        console.warn(`[task-planner] Spec file not found for marker counting: ${specPath}`);
        return 0;
    }
    try {
 ...
 */
export function detectAnyPhaseCompletion(content) {
 ...
 */
export function detectTaskCompletion(content) {
    // Look for patterns like "T1 implemented", "Task 3 complete"
    const taskCompletePattern = /\b(?:T|Task\s*)(\d+)\b.*(?:complete|implemented|done|finished)/i;
    const match = content.match(taskCompletePattern);
    return match ? `T${match[1]}` : null;
}
//# sourceMappingURL=advance-phase.js.map