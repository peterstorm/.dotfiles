 ...
 */
export async function completeWaveGate(taskGraph, stateManager, wave) {
    ...
    if (!testResult.passed) {
        ...
        console.log(ERRORS.WAVE_GATE_TESTS_MISSING(targetWave, testResult.failedTasks));
        return result;
    }
    console.log(`1. Test evidence verified (${waveTasks.length}/${waveTasks.length} tasks):`);
    ...
    if (!newTestResult.passed) {
        ...
        console.log("");
        console.log(ERRORS.WAVE_GATE_NEW_TESTS_MISSING(targetWave, newTestResult.failedTasks));
        return result;
    }
    console.log(`   New tests verified (${waveTasks.length}/${waveTasks.length} tasks):`);
    for (const task of waveTasks) {
        const evidence = task.new_tests_required === false
            ? "not required"
    ...
    if (!reviewResult.passed) {
        ...
        console.log("");
        console.log(ERRORS.WAVE_GATE_REVIEWS_MISSING(targetWave, reviewResult.failedTasks));
        return result;
    }
    ...
    if (taskGraph.spec_check) {
        ...
        if (!specResult.passed) {
            console.log("");
            console.log(`FAILED: Spec alignment has ${taskGraph.spec_check.critical_count} critical findings.`);
        ...
        console.log(`3. Spec alignment verified (verdict: ${taskGraph.spec_check.verdict}).`);
    }
    else {
        console.log("3. Spec alignment: skipped (no spec-check data - run /wave-gate to spawn).");
    ...
    if (!criticalResult.passed) {
        ...
        console.log("");
        console.log(ERRORS.WAVE_GATE_CRITICAL_FINDINGS(targetWave, criticalResult.totalFindings));
        for (const task of waveTasks) {
 ...
 */
export function checkTestEvidence(tasks) {
    const failed = tasks
 ...
 */
export function checkNewTests(tasks) {
    const failed = tasks
        .filter((t) => t.new_tests_required !== false && t.new_tests_written !== true)
        .map((t) => t.id);
    return {
        passed: failed.length === 0,
        failedTasks: failed,
 ...
 */
export function checkReviews(tasks) {
    const failed = tasks
        .filter((t) => t.review_status !== "passed" && t.review_status !== "blocked")
        .map((t) => t.id);
    return {
        passed: failed.length === 0,
        failedTasks: failed,
 ...
 */
export function checkSpecAlignment(taskGraph, wave) {
    if (!taskGraph.spec_check) {
    ...
    if (taskGraph.spec_check.wave !== wave) {
        console.log("");
        console.log(`WARNING: Spec-check was run for wave ${taskGraph.spec_check.wave}, not current wave ${wave}.`);
 ...
 */
export function checkCriticalFindings(tasks) {
    let totalFindings = 0;
    const failed = [];
 ...
 */
export function isWaveImplementationComplete(taskGraph, wave) {
    const waveTasks = taskGraph.tasks.filter((t) => t.wave === wave);
    return waveTasks.every((t) => t.status === "implemented" || t.status === "completed");
}
/**
 * Get wave gate status summary.
 */
export function getWaveGateStatus(taskGraph, wave) {
 ...
 */
export function getMaxWave(taskGraph) {
 ...
 */
export function areAllWavesComplete(taskGraph) {
...
//# sourceMappingURL=complete-wave-gate.js.map