...
RUN="$ROOT/scripts/inference/glm53/run-glm53-flash-exl3-k4-vllm-sm120-v5.sh"
SWITCH="$ROOT/scripts/inference/glm53/switch-glm53-exl3-profile-v5.sh"
V4_RUN="$ROOT/scripts/inference/glm53/run-glm53-flash-exl3-k4-vllm-sm120-v4.sh"
...
for file in "$PULL" "$RUN" "$SWITCH" "$V4_RUN"; do
...
contains "$RUN" '--served-model-name glm-5.3-flash-exl3-k4-vision-mtp-384k'
...
contains "$RUN" 'ai.peterstorm.inference.capacity-evidence=nvfp4-kv-830668-tokens-at-v4-boot'
...
for forbidden in DFLASH_HOST DFLASH_CONTAINER '/draft' 'method":"dflash' 'verify-glm53-flash-dflash2-v1.sh' '--language-model-only' '--enforce-eager'; do
  if grep -Fq -- "$forbidden" "$RUN"; then
    echo "FAIL: MTP v5 contains forbidden DFlash/toggle fragment: $forbidden" >&2
    exit 1
  fi
done
...
contains "$V4_RUN" 'MAX_MODEL_LEN="${MAX_MODEL_LEN:-98304}"'
contains "$V4_RUN" '--served-model-name glm-5.3-flash-exl3-k4-vision-mtp'
contains "$SWITCH" 'TARGET="glm53-flash-exl3-k4-vllm-sm120-v5"'
...
jq -e '
  ...
  select(.id == "glm-5.3-flash-exl3-k4-vision-mtp-384k") |
...
echo 'PASS: GLM-5.3 v84 vision/MTP3 v5 provides immutable 384K capacity with v4 rollback isolation'
