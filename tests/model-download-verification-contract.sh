#!/usr/bin/env bash
# Executable contracts for artifact verification and pinned download completion.
set -euo pipefail
...
PATH="$sandbox/bin:$PATH" inference_quiesce_failed_container author-fixture
export DOCKER_UPDATE_STATUS=1
cleanup_status=0
PATH="$sandbox/bin:$PATH" \
  inference_quiesce_failed_container author-fixture >/dev/null 2>&1 || cleanup_status=$?
unset DOCKER_UPDATE_STATUS
[ "$cleanup_status" -eq 70 ] \
  || fail "incomplete failed-start cleanup must return status 70"
grep -Fq 'docker stop -t 30 author-fixture' "$sandbox/docker-events" \
