...
BLENDER_REF2VA_TEST="$ROOT/tests/minimax-h3-blender-ref2va-workflows-contract.sh"
MUSE_SHEET_TEST="$ROOT/tests/muse-character-sheet-workflows-contract.sh"
RESIZE_TEST="$ROOT/tests/h3-motion-context-resize-contract.sh"
MOTION_CONTEXT_TEST="$ROOT/tests/minimax-h3-motion-context-contract.sh"
FUN_CONTROL_TEST="$ROOT/tests/minimax-h3-fun-controlnet-contract.sh"
SINGULARITY_TEST="$ROOT/tests/minimax-h3-singularity-workflows-contract.sh"
...
for file in "$MODULE" "$DESKTOP" "$NODE" "$NODE_TEST" "$PERSISTENT_HISTORY_NODE" "$PERSISTENT_HISTORY_TEST" "$DOWNLOAD" "$KLEIN_DOWNLOAD" "$REALISM_DOWNLOAD" "$WORKFLOW_BUILDER" "$PIXAROMA_STATE" "$PIXAROMA_STATE_TEST" "$CONTEST_BUILDER" "$CONTEST_BUILDER_TEST" "$H3_DOWNLOAD" "$MUSIC3_DOWNLOAD" "$UPSCALER_DOWNLOAD" "$UPSCALER_TEST" "$DIRECTOR_TEST" "$BLENDER_REF2VA_TEST" "$MUSE_SHEET_TEST" "$RESIZE_TEST" "$MOTION_CONTEXT_TEST" "$FUN_CONTROL_TEST" "$SINGULARITY_TEST" "$CREATIVE_PHASE" "$H3_PHASE" "$ACTIVATE" "$RUNBOOK" "$H3_RUNBOOK" "$ASSETS_RUNBOOK" "$TRANSITION_TEST" "$DOWNLOAD_TEST" "$CREATIVE_PHASE_TEST"; do
  [[ -f "$file" ]] || fail "missing $file"
done
for file in "$NODE_TEST" "$PERSISTENT_HISTORY_TEST" "$DOWNLOAD" "$KLEIN_DOWNLOAD" "$REALISM_DOWNLOAD" "$WORKFLOW_BUILDER" "$PIXAROMA_STATE" "$PIXAROMA_STATE_TEST" "$CONTEST_BUILDER" "$CONTEST_BUILDER_TEST" "$H3_DOWNLOAD" "$MUSIC3_DOWNLOAD" "$UPSCALER_DOWNLOAD" "$UPSCALER_TEST" "$DIRECTOR_TEST" "$BLENDER_REF2VA_TEST" "$MUSE_SHEET_TEST" "$RESIZE_TEST" "$MOTION_CONTEXT_TEST" "$FUN_CONTROL_TEST" "$SINGULARITY_TEST" "$CREATIVE_PHASE" "$H3_PHASE" "$ACTIVATE" "$TRANSITION_TEST" "$DOWNLOAD_TEST" "$CREATIVE_PHASE_TEST"; do
...
bash "$DIRECTOR_TEST"
bash "$BLENDER_REF2VA_TEST"
bash "$MUSE_SHEET_TEST"
bash "$RESIZE_TEST"
bash "$MOTION_CONTEXT_TEST"
bash "$FUN_CONTROL_TEST"
bash "$SINGULARITY_TEST"
...
contains "$MODULE" 'version = "0.34.0";'
contains "$MODULE" 'rev = "12d5279438bfefc058a269eae805ceab6047777f";'
contains "$MODULE" 'hash = "sha256-pW02gtrtWkoPabYe6Q/gicNRM65JRYsc7vtaY1m6H1M=";'
contains "$MODULE" 'version = "0.2.31";'
contains "$MODULE" 'rev = "7c6ca3a5b63857d42c2d49777d6afb69de23f13f";'
contains "$MODULE" 'version = "1.49.6";'
contains "$MODULE" 'version = "0.11.48";'
contains "$MODULE" 'version = "0.1.57";'
contains "$MODULE" 'version = "0.3.322";'
contains "$MODULE" 'version = "0.1.35";'
contains "$MODULE" 'version = "0.5.10";'
contains "$MODULE" 'version = "0.4.15";'
...
contains "$MODULE" 'repo = "ComfyUI-VideoHelperSuite";'
contains "$MODULE" 'rev = "115de7a9d9e34410cffb9ecfd268e993b11a50fb";'
contains "$MODULE" 'hash = "sha256-NgKlqrLOiCoFusVixdDPKYFs1rx4X48TxzVSObxrgl8=";'
contains "$MODULE" 'ln -s ${videoHelperSuiteNode} "$out/ComfyUI-VideoHelperSuite"'
contains "$MODULE" 'repo = "ComfyUI-MMH3Tools";'
contains "$MODULE" 'rev = "7194baedb1cfeb6b55d0797a3af849b73a69c6c2";'
contains "$MODULE" 'hash = "sha256-BhZt+97AaWvSTYTJW6U4agH1N+4YzrCYYwrO9pyS35o=";'
contains "$MODULE" 'pkgs.runCommand "comfyui-mmh3tools-0.87.0-compatible"'
contains "$MODULE" 'from comfy_extras.nodes_minimax_h3 import _encode_ref_audio'
contains "$MODULE" 'vae_sample_rate = getattr(audio_vae, "audio_sample_rate", 32000)'
contains "$MODULE" 'ln -s ${mmh3ToolsNode} "$out/ComfyUI-MMH3Tools"'
contains "$MODULE" 'repo = "Comfyui_Minimax_h3_latent_Upscaler";'
contains "$MODULE" 'rev = "d7c01b9011f2e8439493f6c02c29995a27df276f";'
contains "$MODULE" 'hash = "sha256-QIvU/ioTPw00AzoGsw3h7Y3Pbn7g9gyXWTvdhYULUHk=";'
...
contains "$MODULE" 'h3_safe_upscaler_dir="$user_workflows/minimax-h3-upscaler-local-safe"'
contains "$MODULE" 'blocked_h3_upscaler_dir="$user_workflows/minimax-h3-upscaler-research-only"'
...
contains "$MODULE" 'h3_safe_upscaler_staging="$user_workflows/.minimax-h3-upscaler-local-safe.new"'
...
contains "$MODULE" '"$upscaler_dir" "$h3_safe_upscaler_dir" "$blocked_h3_upscaler_dir"'
contains "$MODULE" 'mv "$h3_safe_upscaler_staging" "$h3_safe_upscaler_dir"'
...
# Prompt-author credentials stay out of workflows and request bodies.
contains "$NODE" 'MUSE_GLIMMER_API_KEY_FILE'
contains "$NODE" 'mode & 0o077'
contains "$NODE" '"reasoning_effort": reasoning_strength,'
contains "$NODE" '"enable_thinking": True,'
contains "$NODE" '"preserve_thinking": True,'
contains "$NODE" 'model": "qwen3.8-27b-blackfrost-abliterated"'
contains "$NODE" 'temperature": 1.0'
contains "$NODE" 'top_p": 0.95'
contains "$NODE" 'top_k": 20'
...
contains "$ACTIVATE" 'GPU_DEVICE=0 PORT="$AUTHOR_PORT" bash "$AUTHOR_LAUNCHER"'
contains "$ACTIVATE" 'lacks the pinned revision marker'
...
contains "$RUNBOOK" 'ComfyUI 0.34.0'
contains "$RUNBOOK" 'workflow templates 0.11.48'
...
contains "$RUNBOOK" '**147 user workflows:**'
...
contains "$RUNBOOK" 'desktop-vllm/qwen3.8-27b-blackfrost-abliterated'
...
contains "$RUNBOOK" 'Blackfrost Qwen Creative Prompt'
...
contains "$RUNBOOK" 'Blackfrost-AI/Qwen3.8-27B-ABLITERATED-BF16'
contains "$RUNBOOK" '9d85770e5eb602322b4bceef55beda357e0bd0ca'
contains "$RUNBOOK" 'the upstream safety-stock checkpoint'
contains "$RUNBOOK" '55,586,061,398 bytes'
