...
for file in "$MODULE" "$DESKTOP" "$NODE" "$NODE_TEST" "$PERSISTENT_HISTORY_NODE" "$PERSISTENT_HISTORY_TEST" "$DOWNLOAD" "$KLEIN_DOWNLOAD" "$REALISM_DOWNLOAD" "$WORKFLOW_BUILDER" "$PIXAROMA_STATE" "$PIXAROMA_STATE_TEST" "$CONTEST_BUILDER" "$CONTEST_BUILDER_TEST" "$H3_DOWNLOAD" "$MUSIC3_DOWNLOAD" "$UPSCALER_DOWNLOAD" "$UPSCALER_TEST" "$DIRECTOR_TEST" "$BLENDER_REF2VA_TEST" "$MOTION_CONTEXT_TEST" "$CREATIVE_PHASE" "$H3_PHASE" "$ACTIVATE" "$RUNBOOK" "$H3_RUNBOOK" "$ASSETS_RUNBOOK" "$TRANSITION_TEST" "$DOWNLOAD_TEST" "$CREATIVE_PHASE_TEST"; do
  [[ -f "$file" ]] || fail "missing $file"
done
for file in "$NODE_TEST" "$PERSISTENT_HISTORY_TEST" "$DOWNLOAD" "$KLEIN_DOWNLOAD" "$REALISM_DOWNLOAD" "$WORKFLOW_BUILDER" "$PIXAROMA_STATE" "$PIXAROMA_STATE_TEST" "$CONTEST_BUILDER" "$CONTEST_BUILDER_TEST" "$H3_DOWNLOAD" "$MUSIC3_DOWNLOAD" "$UPSCALER_DOWNLOAD" "$UPSCALER_TEST" "$DIRECTOR_TEST" "$BLENDER_REF2VA_TEST" "$MOTION_CONTEXT_TEST" "$CREATIVE_PHASE" "$H3_PHASE" "$ACTIVATE" "$TRANSITION_TEST" "$DOWNLOAD_TEST" "$CREATIVE_PHASE_TEST"; do
