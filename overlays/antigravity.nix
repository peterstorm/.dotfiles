...
let
  ...
  source = information.sources.${stdenv.hostPlatform.system}
    or (throw "antigravity: unsupported system ${stdenv.hostPlatform.system}");
in {
  antigravity = prev.antigravity.overrideAttrs (oldAttrs: {
    version = information.version;
    src = prev.fetchurl { inherit (source) url sha256; };
  });
}
