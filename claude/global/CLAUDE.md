...
1. **Load, don't skim.** Read `~/dev/claude-plugins/loom/rules/architecture.md` in FULL (home-relative — resolves on any machine; same dir the loom-rules-gate defaults to) — a read with a `limit:` argument does not count as loaded (the gate counts it; that is a loophole, not a contract). Same for the language rule in scope (`typescript-patterns.md`, `java-patterns.md`, `rust-patterns.md`; add `property-testing.md` when adding business-rule tests).
