#!/usr/bin/env bash
# Build the workstation BF16 V1.7 adaptation of the upstream Muse Minimax
...
#     and Balanced collapse to (disabled), and the plain subgraph loaders
#     match. The one load-bearing LoRA is the workstation turbo 8-step file;
#     optional creator style LoRAs are stripped
...
jq \
  ...
  | (.nodes[] | select(.id == 423)) |= (
      ...
      | .widgets_values[5] = "(disabled)"
      | .widgets_values_named.vae_name_2 = "(disabled)"
    )

  # Strip the same serialized LoRA and inactive LTX2.3 selectors inside the
  # bypassed subgraph.
  | (.definitions.subgraphs[].nodes[] | select(.id == 416)) |= (
      .widgets_values[0] = "(disabled)"
      | .widgets_values_named.vae_name = "(disabled)"
    )
  ...
  | (.nodes[] | select(.id == 388) | .widgets_values[26]) |= (
      ...
      | .prompt_gen_model = "glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v11.1"
      | tojson
    )
...
jq -e --arg status_note "$status_note" '
  ...
  and ([.nodes[] | select(.id == 423)
      | [.mode, .widgets_values[5], .widgets_values_named.vae_name_2,
         .widgets_values[6:12]]]
    == [[4, "(disabled)", "(disabled)",
            ["minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors",
             "(disabled)", "(disabled)",
  ...
  and ([.definitions.subgraphs[].nodes[] | select(.id == 416)
      | [.widgets_values[0], .widgets_values_named.vae_name]]
    == [["(disabled)", "(disabled)"]])
...
if grep -RqiE \
  'easy showAnything|iToolsPreviewText|"SaveTextWithPath"|LayerUtility: PurgeVRAM|int8|nvfp4|curve-Q4|curve-Q5|Q4_K_M|mmproj|resolve/main|tree/main|joeygambino|lightx2v|taeltx2_3|H3\\\\|MysticXXX|GalaxyAce' \
  "$output_dir"; then
