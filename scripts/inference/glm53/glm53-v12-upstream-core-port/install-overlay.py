#!/usr/bin/env python
"""Install the pinned upstream-core-port v12 overlay into the served trees.

Identical policy to v11.1's installer: the manifest is the only install list,
its mapping/destination counts are pinned, every destination is classified as a
...
v12 changes three overlay files, adds gpu_worker.py as a new overlay file, and
extends MANIFEST.txt by one line, so the pinned counts move from v11.1's
(99 mappings / 101 destinations / 91 replacements) to 100 / 102 / 92; a
differing count means the vendored archive is not the v12 overlay this
installer was written for.
"""
...
EXPECTED_DESTINATIONS = 102
EXPECTED_REPLACEMENTS = 92
EXPECTED_ADDITIONS = 10
# The three r2.1 fix sites plus the four v12 port sites must be present in the
# overlay being installed, or this is an r2.1 archive wearing a v12 label.
FIX_MARKERS = (
...
def main() -> None:
    ...
    print(
        f"V12 OVERLAY INSTALLED: {len(mappings)} installs "
        ...
        f"{len(FIX_MARKERS)} fix markers present (3 r2.1 + 4 v12)"
    )
