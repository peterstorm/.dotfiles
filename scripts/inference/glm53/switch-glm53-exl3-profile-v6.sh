#!/usr/bin/env bash
# Safely start/stop the text-only FP8-DS-MLA MTP3 384K v6 profile with rollback.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v6"
EXPECTED_MODEL="glm-5.3-flash-exl3-k4-text-fp8kv-mtp-384k"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v6.sh"
MODE="${1:-status}"
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: text FP8-DS-MLA v6 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target; then
      ...
      echo "error: text FP8-DS-MLA v6 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
