#!/usr/bin/env bash
# Safely start/stop the multimodal NVFP4 MTP3 384K fair-prefill v7 profile with rollback.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v7"
EXPECTED_MODEL="glm-5.3-flash-exl3-k4-vision-mtp-384k-fair-v7"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v7.sh"
MODE="${1:-status}"
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: multimodal NVFP4 fair-prefill v7 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target; then
      ...
      echo "error: multimodal NVFP4 fair-prefill v7 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
