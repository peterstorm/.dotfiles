#!/usr/bin/env bash
# Long-context multimodal TP2/EP2/DCP2 v84 profile with built-in MTP3 for GLM-5.3 EXL3 K4.
set -euo pipefail
...
CACHE_HOST="${CACHE_HOST:-/models/vllm-cache/glm53-flash-exl3-k4-sm120-v5}"
NAME="glm53-flash-exl3-k4-vllm-sm120-v5"
...
[ "$GPU_MEMORY_UTILIZATION" = 0.986 ] || {
  echo "error: long-context multimodal MTP v5 fixes GPU_MEMORY_UTILIZATION at 0.986" >&2
  exit 2
}
...
if [ "$MODE" = --preflight ]; then
  echo "GLM-5.3 EXL3 K4 v84 MTP3 384K multimodal checkpoint and image preflight: PASS"
  exit 0
fi
...
ENVFILE="$INFERENCE_OPERATOR_HOME/.config/glm53/exl3-k4-vllm-sm120-v5.env"
...
docker run -d --init \
  ...
  --name "$NAME" \
  --label ai.peterstorm.inference.profile=glm53-flash-exl3-k4-vllm-sm120-v5 \
  --label ai.peterstorm.inference.capacity-evidence=nvfp4-kv-830668-tokens-at-v4-boot \
  ...
  --served-model-name glm-5.3-flash-exl3-k4-vision-mtp-384k \
  --host 0.0.0.0 \
  --port 8000 \
...
printf '%s\n' 'v5 uses TP2/EP2/DCP2, 393,216 context, built-in MTP3, calibrated NVFP4 MLA KV, TORCH_SDPA vision, four images, and no video.'
