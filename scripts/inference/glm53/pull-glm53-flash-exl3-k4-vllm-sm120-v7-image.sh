#!/usr/bin/env bash
# Pull and prove the v84 multimodal NVFP4 image and fair-prefill scheduler capability for v7.
set -euo pipefail
...
IDENTITY_FILE="${IDENTITY_FILE:-$HOME/.local/state/glm53/exl3-k4-vllm-sm120-v7-image.identity}"
...
paths = {
    "triton_attention_helpers.py": root / "v1/attention/ops/triton_attention_helpers.py",
    "int4_per_token_head.py": root / "v1/attention/ops/int4_per_token_head.py",
...
print("GLM-5.3 v84 multimodal + NVFP4 sparse MLA + long-prefill cap capability probe: PASS")
PY
...
profile_capabilities=multimodal,nvfp4_ds_mla,b12x_mla_sparse,prefix_cache,long_prefill_token_threshold,mtp3
EOF_IDENTITY
