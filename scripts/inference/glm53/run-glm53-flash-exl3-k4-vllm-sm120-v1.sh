#!/usr/bin/env bash
# Experimental text-only TP2 vLLM profile for GLM-5.3 EXL3 K4 on RTX PRO 6000 SM120.
set -euo pipefail
...
IMAGE_CONFIG="sha256:19a51d921523dd0c21afbf99bb49a00fc2d3feb6f565b1d3474ed0120372d847"
MODEL_HOST="${MODEL_HOST:-/models/GLM-5.3-Flash-EXL3-K4-v1}"
MODEL_CONTAINER="/models/brandonmusic/GLM-5.3-Flash-EXL3-K4-v1"
CACHE_HOST="${CACHE_HOST:-/models/vllm-cache/glm53-flash-exl3-k4-sm120-v1}"
NAME="glm53-flash-exl3-k4-vllm-sm120-v1"
...
integer_in_range MAX_MODEL_LEN "$MAX_MODEL_LEN" 1 131072
integer_in_range MAX_NUM_SEQS "$MAX_NUM_SEQS" 1 8
integer_in_range MAX_NUM_BATCHED_TOKENS "$MAX_NUM_BATCHED_TOKENS" 1 32768
...
if ! actual_image_id="$(docker image inspect "$IMAGE" --format '{{.Id}}' 2>/dev/null)"; then
  echo "error: pinned EXL3 image is absent; run scripts/inference/glm53/pull-glm53-flash-exl3-k4-vllm-sm120-v1-image.sh" >&2
  exit 1
fi
[ "$actual_image_id" = "$IMAGE_CONFIG" ] || {
  echo "error: local EXL3 image config is $actual_image_id, expected $IMAGE_CONFIG" >&2
  exit 1
}
MODEL_HOST="$MODEL_HOST" "$SCRIPT_DIR/verify-glm53-flash-exl3-k4-v1.sh"
if [ "$MODE" = --preflight ]; then
  echo "GLM-5.3 EXL3 K4 vLLM SM120 v1 checkpoint and image preflight: PASS"
  exit 0
fi
...
ENVFILE="$INFERENCE_OPERATOR_HOME/.config/glm53/exl3-k4-vllm-sm120-v1.env"
...
docker run -d --init \
  ...
  --name "$NAME" \
  --label ai.peterstorm.inference.profile=glm53-flash-exl3-k4-vllm-sm120-v1 \
  ...
  -e HF_HUB_OFFLINE=1 \
  -e VLLM_ATTENTION_BACKEND=B12X_MLA_SPARSE \
...
printf '%s\n' 'Initial qualification is text-only, C1, MTP-off, eager, and capped at 128K.'
