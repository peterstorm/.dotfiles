#!/usr/bin/env bash
# GLM-5.3 v13: v12 plus the grammar redesign port and two upstream hardening
# fixes.
#
# Same serving shape as v12 (native FP8 KV, vision, MTP3, TP2/EP/DCP2 a2a); the
# image differs in four new overlay files replacing base-image files. What v13
# adds to the operator surface:
#   The v11.1/v12 crash mode is fixed: structured/guided decoding combined
#     with MTP3 speculative decoding no longer kills the engine in
#     _build_grammar_row_mapping (PR #52477's fixed-width stride replaces the
#     CPU-side compaction assertion with device-side logit counts). The
#     structured/tool-call + MTP-3 traffic that crashed v11.1/v12 is now a
#     legal serving shape — but its acceptance is still an operator gate
#     (glm53-gates.py mixed-traffic with structured/tool-call + MTP-3, plus
#     the GPU grammar-correctness gate) before promotion.
#   upstream #53046/#55455 add no knobs: validate-before-accept is producer
#     behavior, warmup defer is worker behavior.
#   CUBLAS_WORKSPACE_CONFIG=:4096:1, VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS=0,
#     VLLM_MAMBA_STATE_PROTECT/_AGE, and the never-set
#     VLLM_MAMBA_ALIGN_CAP_LEGACY kill-switch are all v12's semantics.
# Launch and qualification gates (GPU serving tests, mixed-traffic gate,
# >=24h soak) remain operator steps before promotion.
#
...
# A fresh cache directory: v12's carries torch/triton artifacts compiled
# against the v12 tree. Nothing in it is worth inheriting.
CACHE_HOST="${CACHE_HOST:-/models/vllm-cache/glm53-flash-exl3-k4-sm120-v13}"
NAME="glm53-flash-exl3-k4-vllm-sm120-v13"
SERVED_MODEL="glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v13"
...
# The workstation GPU cap was retuned to 400 W (nvidia-power-limit.service,
# verified live on both GPUs); v12's 450 W would fail the cap check.
EXPECTED_POWER_LIMIT_WATTS="400"
MODE="${1:---launch}"
...
if [ "$IMAGE_CONFIG" = "unrecorded" ]; then
  echo "error: the v13 image id is not recorded; build it with scripts/inference/glm53/pull-glm53-flash-exl3-k4-vllm-sm120-v13-image.sh and record derived_id as IMAGE_CONFIG in ${0##*/}" >&2
  exit 2
fi
...
if ! actual_image_id="$(docker image inspect "$IMAGE" --format '{{.Id}}' 2>/dev/null)"; then
  echo "error: pinned GLM v13 image is absent; run scripts/inference/glm53/pull-glm53-flash-exl3-k4-vllm-sm120-v13-image.sh" >&2
  exit 1
fi
[ "$actual_image_id" = "$IMAGE_CONFIG" ] || {
  echo "error: local GLM v13 image config is $actual_image_id, expected $IMAGE_CONFIG" >&2
  exit 1
}
overlay_release="$(docker image inspect "$IMAGE" \
  --format '{{index .Config.Labels "ai.peterstorm.inference.upstream-core-port.release"}}')"
[ "$overlay_release" = v13 ] || {
  echo "error: image does not carry the upstream-core-port v13 overlay (label: ${overlay_release:-<missing>})" >&2
  exit 1
}
...
overlay_source_port="$(docker image inspect "$IMAGE" \
  --format '{{index .Config.Labels "ai.peterstorm.inference.upstream-core-port.source-port"}}')"
[ "$overlay_source_port" = "$SOURCE_PORT" ] || {
  echo "error: image records source-port ${overlay_source_port:-<missing>}, expected $SOURCE_PORT" >&2
  exit 1
}
MODEL_HOST="$MODEL_HOST" "$SCRIPT_DIR/verify-glm53-flash-exl3-k4-v1.sh"
if [ "$MODE" = --preflight ]; then
  echo "GLM-5.3 v13 upstream-core-port r2.1+ported+grammar native-FP8 multimodal preflight: PASS"
...
if ss -H -ltn | awk '{print $4}' | grep -Eq '(^|:)8000$'; then
  echo "error: host port 8000 is already listening; use the v13 switcher when ready to replace the active profile" >&2
  exit 1
fi
...
ENVFILE="$INFERENCE_OPERATOR_HOME/.config/glm53/exl3-k4-vllm-sm120-v13.env"
...
docker run -d --init \
  --restart no \
  --name "$NAME" \
  --label ai.peterstorm.inference.profile=glm53-flash-exl3-k4-vllm-sm120-v13 \
  --label ai.peterstorm.inference.capacity-evidence=must-be-recorded-from-each-v13-boot \
  --label ai.peterstorm.inference.checkpoint=brandonmusic/GLM-5.3-Flash-EXL3-4bpw@4739eb1bcfd478e8a32da6358908567bc3a9ac51 \
  --label ai.peterstorm.inference.upstream-core-port="legend-r2.1-$UPSTREAM_COMMIT+grammar-port" \
  --label ai.peterstorm.inference.source-port="$SOURCE_PORT" \
...
printf "Started upstream-core-port r2.1+ported+grammar native-FP8 multimodal profile '%s'. Follow: docker logs -f %s\n" "$NAME" "$NAME"
printf "API key: %s (send as 'Authorization: Bearer <key>')\n" "$KEYFILE"
printf '%s\n' 'v13 is v12 plus the grammar redesign port: PR #52477 fixed-width stride replaces the _build_grammar_row_mapping crash (structured decoding + MTP3 no longer kills the engine), upstream #53046 FSM validate-before-accept for reasoning models ending mid-window, upstream #55455 warmup defers adaptive verification.'
...
printf '%s\n' 'The crash-mode fix makes structured/tool-call + MTP-3 traffic legal, but its acceptance is still an operator gate: glm53-gates.py mixed-traffic with structured/tool-call + MTP-3 plus the GPU grammar-correctness gate, then >=24h soak, before promotion.'
