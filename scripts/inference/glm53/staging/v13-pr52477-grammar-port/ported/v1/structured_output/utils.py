...
def apply_grammar_bitmask(
    ...
    # ([PR52477-PORT]: mask_stride rows per request, GPU-count-driven).
    ...
    # [PR52477-PORT] Fixed-width blocks: each grammar request owns mask_stride
    # source rows, so the read offset is grammar_idx * mask_stride. This drops
    # the running compaction cursor and the scheduler-count stride, which could
    # disagree with the GPU's finalized draft counts under adaptive verification.
    ...
    for grammar_idx, req_id in enumerate(grammar_output.structured_output_request_ids):
        ...
        if (logit_idx := struct_out_req_batch_indices.get(req_id)) is not None:
            for i in range(1 + num_worker_spec_tokens):
