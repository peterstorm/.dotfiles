#!/usr/bin/env bash
# Safely start/stop the multimodal FP8-DS-MLA MTP3 359K v8 profile with rollback.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v8"
EXPECTED_MODEL="glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v8"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v8.sh"
MODE="${1:-status}"
STARTUP_TIMEOUT_SECONDS="${STARTUP_TIMEOUT_SECONDS:-3600}"
KV_RECEIPT="${KV_RECEIPT:-$HOME/.local/state/glm53/exl3-k4-vllm-sm120-v8-kv-capacity.txt}"
...
report_kv_capacity() {
  ...
  if [ -z "$memory_line" ] || [ -z "$capacity_line" ]; then
    echo "error: authenticated v8 boot did not emit complete KV-capacity evidence" >&2
    return 1
  fi
  ...
  receipt_tmp="$(mktemp "$receipt_dir/.v8-kv-capacity.XXXXXX")"
  {
    printf 'container=%s\nmodel=%s\nstarted_at=%s\nimage=%s\n' \
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: multimodal FP8-DS-MLA v8 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target || ! report_kv_capacity || ! promote_restart_policy; then
      ...
      echo "error: multimodal FP8-DS-MLA v8 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
