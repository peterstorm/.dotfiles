#!/usr/bin/env bash
# Safely start/stop the unpromoted graph-enabled v9 candidate with rollback.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v9"
EXPECTED_MODEL="glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v9"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v9.sh"
MODE="${1:-status}"
STARTUP_TIMEOUT_SECONDS="${STARTUP_TIMEOUT_SECONDS:-3600}"
KV_RECEIPT="${KV_RECEIPT:-$HOME/.local/state/glm53/exl3-k4-vllm-sm120-v9-kv-capacity.txt}"
...
report_kv_capacity() {
  local logs memory_line capacity_line started_at image_id receipt_dir receipt_tmp
  logs="$(docker logs "$TARGET" 2>&1)" || return 1
  memory_line="$(grep -F 'Available KV cache memory:' <<<"$logs" | tail -n 1)"
  capacity_line="$(grep -F 'GPU KV cache size:' <<<"$logs" | tail -n 1)"
  if [ -z "$memory_line" ] || [ -z "$capacity_line" ]; then
    echo "error: authenticated v9 boot did not emit complete KV-capacity evidence" >&2
    return 1
  fi
  ...
  receipt_tmp="$(mktemp "$receipt_dir/.v9-kv-capacity.XXXXXX")"
  {
    printf 'container=%s\nmodel=%s\nstarted_at=%s\nimage=%s\n' \
...
retain_candidate_restart_policy() {
  ...
  [ "$restart_policy" = no ] || {
    echo "error: unpromoted v9 candidate restart policy is $restart_policy, expected no" >&2
    return 1
  }
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: multimodal FP8-DS-MLA v9 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target || ! report_kv_capacity || ! retain_candidate_restart_policy; then
      ...
      echo "error: multimodal FP8-DS-MLA v9 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
