#!/usr/bin/env bash
# Safely start/stop the unpromoted v11 upstream-core-port r2 candidate.
# v11 carries native FP8 KV (528-byte GLM_NOPE records) and vision; it replaces
# the v9/v10 patch series rather than extending it, so rollback targets the
# still-present v10 container.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v11"
EXPECTED_MODEL="glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v11"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v11.sh"
MODE="${1:-status}"
STARTUP_TIMEOUT_SECONDS="${STARTUP_TIMEOUT_SECONDS:-3600}"
KV_RECEIPT="${KV_RECEIPT:-$HOME/.local/state/glm53/exl3-k4-vllm-sm120-v11-kv-capacity.txt}"
...
report_kv_capacity() {
  local logs kv_dtype retention protect capacity_line started_at image_id receipt_dir receipt_tmp
  logs="$(docker logs "$TARGET" 2>&1)" || return 1
  # v11 sizes KV from --gpu-memory-utilization rather than v10's fixed byte
  # budget, so the receipt pins the dtype and the two cache-behaviour settings
  # the upstream fixes are coupled to, alongside this boot's exact allocation.
  ...
  [ "$kv_dtype" = fp8_ds_mla-glm-nope-528b ] || {
    echo "error: authenticated v11 boot reports KV dtype ${kv_dtype:-<missing>}, expected fp8_ds_mla-glm-nope-528b" >&2
    return 1
  }
  ...
  if [ -z "$capacity_line" ]; then
    echo "error: authenticated v11 boot did not emit complete KV-capacity evidence" >&2
    return 1
  fi
  ...
  receipt_tmp="$(mktemp "$receipt_dir/.v11-kv-capacity.XXXXXX")"
  {
    printf 'container=%s\nmodel=%s\nstarted_at=%s\nimage=%s\n' \
      "$TARGET" "$EXPECTED_MODEL" "$started_at" "$image_id"
    printf 'kv_cache_dtype=%s\nprefix_cache_retention_interval=%s\nmamba_state_protect=%s\n' \
      "$kv_dtype" "$retention" "$protect"
    printf '%s\n' "$capacity_line"
  } >"$receipt_tmp"
  chmod 600 "$receipt_tmp"
  mv -f "$receipt_tmp" "$KV_RECEIPT"
  printf 'KV CACHE BOOT RECEIPT (%s):\nkv_cache_dtype=%s retention=%s mamba_state_protect=%s\n%s\n' \
    "$KV_RECEIPT" "$kv_dtype" "$retention" "$protect" "$capacity_line"
}
...
retain_candidate_restart_policy() {
  ...
  [ "$restart_policy" = no ] || {
    echo "error: unpromoted v11 candidate restart policy is $restart_policy, expected no" >&2
    return 1
  }
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: multimodal FP8-DS-MLA v11 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target || ! report_kv_capacity || ! retain_candidate_restart_policy; then
      ...
      echo "error: multimodal FP8-DS-MLA v11 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
