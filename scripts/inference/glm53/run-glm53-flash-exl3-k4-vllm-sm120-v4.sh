#!/usr/bin/env bash
# Multimodal TP2/EP2/DCP2 v84 profile with built-in MTP3 for GLM-5.3 EXL3 K4.
set -euo pipefail
...
CACHE_HOST="${CACHE_HOST:-/models/vllm-cache/glm53-flash-exl3-k4-sm120-v4}"
NAME="glm53-flash-exl3-k4-vllm-sm120-v4"
...
[ "$GPU_MEMORY_UTILIZATION" = 0.986 ] || {
  echo "error: multimodal MTP v4 fixes GPU_MEMORY_UTILIZATION at 0.986" >&2
  exit 2
}
...
if [ "$MODE" = --preflight ]; then
  echo "GLM-5.3 EXL3 K4 v84 MTP3 multimodal checkpoint and image preflight: PASS"
  exit 0
fi
...
ENVFILE="$INFERENCE_OPERATOR_HOME/.config/glm53/exl3-k4-vllm-sm120-v4.env"
...
docker run -d --init \
  ...
  --name "$NAME" \
  --label ai.peterstorm.inference.profile=glm53-flash-exl3-k4-vllm-sm120-v4 \
  ...
  --served-model-name glm-5.3-flash-exl3-k4-vision-mtp \
  --host 0.0.0.0 \
  --port 8000 \
...
printf "Started multimodal MTP3 v84 profile '%s'. Follow: docker logs -f %s\n" "$NAME" "$NAME"
printf "API key: %s (send as 'Authorization: Bearer <key>')\n" "$KEYFILE"
printf '%s\n' 'v4 uses TP2/EP2/DCP2, 98,304 context, built-in MTP3, TORCH_SDPA vision, four images, and no video.'
