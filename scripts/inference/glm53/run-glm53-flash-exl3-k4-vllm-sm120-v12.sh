#!/usr/bin/env bash
# GLM-5.3 v12: upstream-core-port r2.1 + the four changes ported from the Spark
# TP2 source-locked image (2026-09-13 R2 update).
#
# Same serving shape as v11.1 (native FP8 KV, vision, MTP3, TP2/EP/DCP2 a2a);
# the image differs in three modified overlay files plus the new gpu_worker.py
# overlay file. What v12 adds to the operator surface:
#   CUBLAS_WORKSPACE_CONFIG=:4096:1 — the R2-measured 4 MiB cuBLAS workspace
#     saves ~140 MiB per rank on SM120 (reversible launcher setting).
#   VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS stays 0 (v11.1's setting): with
#     PR #694 the profile peak is activation-only and the estimate is billed
#     separately, so flag off keeps behavior identical to v11.1 and flag on
#     fixes the double-count.
#   PR #710/#718 are inert off SM120 or outside align mode; no new knobs.
#   VLLM_MAMBA_STATE_PROTECT / _AGE / VLLM_MAMBA_ALIGN_CAP_LEGACY semantics are
#     v11.1's; the legacy kill-switch is still deliberately never set here.
...
# Recorded from pull-glm53-flash-exl3-k4-vllm-sm120-v12-image.sh's derived_id
# on the serving host. Until then the profile is unlaunchable by construction.
IMAGE_CONFIG="sha256:a8b8c5bd83c8df4112fd1badfbcdd6cb4e8a8bea7b3a03508f4f9dda5b67f6eb"
IMAGE="$IMAGE_CONFIG"
MODEL_HOST="${MODEL_HOST:-$HOME/models/GLM-5.3-Flash-EXL3-K4-v1}"
MODEL_CONTAINER="/model"
# A fresh cache directory: v11.1's carries torch/triton artifacts compiled
# against the r2.1 tree. Nothing in it is worth inheriting.
CACHE_HOST="${CACHE_HOST:-/models/vllm-cache/glm53-flash-exl3-k4-sm120-v12}"
NAME="glm53-flash-exl3-k4-vllm-sm120-v12"
SERVED_MODEL="glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v12"
...
if [ "$IMAGE_CONFIG" = "unrecorded" ]; then
  echo "error: the v12 image id is not recorded; build it with scripts/inference/glm53/pull-glm53-flash-exl3-k4-vllm-sm120-v12-image.sh and record derived_id as IMAGE_CONFIG in ${0##*/}" >&2
  exit 2
fi
...
if ! actual_image_id="$(docker image inspect "$IMAGE" --format '{{.Id}}' 2>/dev/null)"; then
  echo "error: pinned GLM v12 image is absent; run scripts/inference/glm53/pull-glm53-flash-exl3-k4-vllm-sm120-v12-image.sh" >&2
  exit 1
fi
[ "$actual_image_id" = "$IMAGE_CONFIG" ] || {
  echo "error: local GLM v12 image config is $actual_image_id, expected $IMAGE_CONFIG" >&2
  exit 1
}
overlay_release="$(docker image inspect "$IMAGE" \
  --format '{{index .Config.Labels "ai.peterstorm.inference.upstream-core-port.release"}}')"
[ "$overlay_release" = v12 ] || {
  echo "error: image does not carry the upstream-core-port v12 overlay (label: ${overlay_release:-<missing>})" >&2
  exit 1
}
...
if [ "$MODE" = --preflight ]; then
  echo "GLM-5.3 v12 upstream-core-port r2.1+ported native-FP8 multimodal preflight: PASS"
...
if ss -H -ltn | awk '{print $4}' | grep -Eq '(^|:)8000$'; then
  echo "error: host port 8000 is already listening; use the v12 switcher when ready to replace the active profile" >&2
  exit 1
fi
...
ENVFILE="$INFERENCE_OPERATOR_HOME/.config/glm53/exl3-k4-vllm-sm120-v12.env"
...
docker run -d --init \
  --restart no \
  --name "$NAME" \
  --label ai.peterstorm.inference.profile=glm53-flash-exl3-k4-vllm-sm120-v12 \
  --label ai.peterstorm.inference.capacity-evidence=must-be-recorded-from-each-v12-boot \
  --label ai.peterstorm.inference.checkpoint=brandonmusic/GLM-5.3-Flash-EXL3-4bpw@4739eb1bcfd478e8a32da6358908567bc3a9ac51 \
  --label ai.peterstorm.inference.upstream-core-port="legend-r2.1-$UPSTREAM_COMMIT+ported" \
...
printf "Started upstream-core-port r2.1+ported native-FP8 multimodal profile '%s'. Follow: docker logs -f %s\n" "$NAME" "$NAME"
printf "API key: %s (send as 'Authorization: Bearer <key>')\n" "$KEYFILE"
printf '%s\n' 'v12 is v11.1 (legend r2.1) plus the Spark TP2 port: PR #710 SM120/121 disjoint-batch BMM, PR #718 Mamba null-gap recurrent-state cleanup, PR #694 graph-memory double-count fix, and CUBLAS_WORKSPACE_CONFIG=:4096:1 (~140 MiB/rank saved on SM120).'
...
printf '%s\n' 'PR #710/#718 are inert off SM120 or outside align mode; PR #694 keeps VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS=0 behavior identical to v11.1. The v12 switcher records exact KV capacity, verifies authenticated exact-model readiness, then promotes the accepted container to restart=unless-stopped.'
