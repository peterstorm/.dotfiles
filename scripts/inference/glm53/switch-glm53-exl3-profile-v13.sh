#!/usr/bin/env bash
# Safely start/stop the promoted v13 upstream-core-port r2.1+ported+grammar
# profile. v13 is v12 plus the grammar redesign port (PR #52477 fixed-width
# stride replacing the _build_grammar_row_mapping crash, upstream #53046 FSM
# validate-before-accept, upstream #55455 warmup defer). The rollback target is
# whatever profile set was running before — v11.1 is the qualified release a
# failed v13 acceptance must never be conflated with. Launch remains
# transactional at restart=no; only a fully accepted target is promoted.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v13"
EXPECTED_MODEL="glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v13"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v13.sh"
...
KV_RECEIPT="${KV_RECEIPT:-$HOME/.local/state/glm53/exl3-k4-vllm-sm120-v13-kv-capacity.txt}"
...
report_kv_capacity() {
  ...
  # v13 sizes KV from --gpu-memory-utilization like v11.1/v12, so the receipt
  # pins the dtype and the cache-behaviour settings the upstream fixes are
  # coupled to, the reserve aging bound, and this boot's port provenance,
  # alongside this boot's exact allocation.
  ...
  [ "$kv_dtype" = fp8_ds_mla-glm-nope-528b ] || {
    echo "error: authenticated v13 boot reports KV dtype ${kv_dtype:-<missing>}, expected fp8_ds_mla-glm-nope-528b" >&2
    return 1
  }
  ...
  [ "$source_port" = "$SOURCE_PORT" ] || {
    echo "error: authenticated v13 boot reports source-port ${source_port:-<missing>}, expected $SOURCE_PORT" >&2
    return 1
  }
  capacity_line="$(grep -F 'GPU KV cache size:' <<<"$logs" | tail -n 1)"
  if [ -z "$capacity_line" ]; then
    echo "error: authenticated v13 boot did not emit complete KV-capacity evidence" >&2
    return 1
  fi
  ...
  receipt_tmp="$(mktemp "$receipt_dir/.v13-kv-capacity.XXXXXX")"
  {
    printf 'container=%s\nmodel=%s\nstarted_at=%s\nimage=%s\n' \
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: multimodal FP8-DS-MLA v13 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target || ! report_kv_capacity || ! promote_restart_policy; then
      ...
      echo "error: multimodal FP8-DS-MLA v13 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
