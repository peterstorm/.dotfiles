#!/usr/bin/env bash
# Safely start/stop the multimodal MTP3 GLM-5.3 EXL3 K4 v84 profile with rollback.
set -euo pipefail
...
TARGET="glm53-flash-exl3-k4-vllm-sm120-v4"
RUN="$SCRIPT_DIR/run-glm53-flash-exl3-k4-vllm-sm120-v4.sh"
MODE="${1:-status}"
...
case "$MODE" in
  ...
  start)
    ...
    if ! "$RUN" --launch; then
      echo "error: multimodal EXL3 launch failed; restoring previous profile set" >&2
    ...
    if ! wait_for_target; then
      ...
      echo "error: multimodal EXL3 acceptance failed; restoring previous profile set" >&2
      restore_profiles "${previous[@]}" || true
      exit 1
    fi
    ;;
