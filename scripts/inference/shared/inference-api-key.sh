#!/usr/bin/env bash
# Shared credential handling for the mutually exclusive inference servers on :8000.
...
# Resolves one endpoint credential and synchronizes both historical key paths.
# Priority is explicit value -> DeepSeek key -> Qwen key -> sops fallback -> generated
# key. Pi uses the same file order, so every launcher and client converge on one value.
