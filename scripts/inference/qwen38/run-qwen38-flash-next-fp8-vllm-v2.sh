#!/usr/bin/env bash
# Reconstructible TP2 Qwen3.8 Flash-Next FP8 profile with UVA PLE and exact persistent top-k.
set -euo pipefail
...
IMAGE_CONFIG="sha256:931c3c595e48f63c1900ee559966cad845673e37bdc2bd73ce5f49390a8154e1"
MODEL_HOST="${MODEL_HOST:-$HOME/models/Qwen3.8-Flash-Next-FP8-v1}"
MODEL_CONTAINER="/model"
CACHE_HOST="${CACHE_HOST:-$HOME/.cache/vllm/qwen38-flash-next-fp8-v2}"
TMP_HOST="${TMP_HOST:-$CACHE_HOST/tmp}"
NAME="qwen38-flash-next-fp8-vllm-v2"
...
if ! actual_image_id="$(docker image inspect "$IMAGE" --format '{{.Id}}' 2>/dev/null)"; then
  echo "error: pinned Flash-Next v2 image is absent; run scripts/inference/qwen38/pull-qwen38-flash-next-vllm-v2-image.sh" >&2
  exit 1
fi
[ "$actual_image_id" = "$IMAGE_CONFIG" ] || {
  echo "error: local Flash-Next v2 image config is $actual_image_id, expected $IMAGE_CONFIG" >&2
  exit 1
}
docker image inspect "$IMAGE" --format '{{json .Config.Labels}}' | jq -e \
  --arg source "e126687a9a828d513c01a07cd69f025f27d63280" \
  --arg overlay "c0ac28980016af357df50359d301648352eebbf2" '
    .["ai.peterstorm.inference.overlay"] == "qwen38-flash-next-v2-safe" and
    .["ai.peterstorm.inference.source-commit"] == $source and
    .["ai.peterstorm.inference.source-overlay-commit"] == $overlay and
    .["ai.peterstorm.inference.prefix-cache"] == "disabled-until-equivalence-qualified"
  ' >/dev/null || {
    echo "error: Flash-Next v2 image provenance labels differ" >&2
    exit 1
  }
MODEL_HOST="$MODEL_HOST" "$SCRIPT_DIR/verify-qwen38-flash-next-fp8-v1.sh"
if [ "$MODE" = --preflight ]; then
  echo "Qwen3.8 Flash-Next FP8 vLLM v2 checkpoint and image preflight: PASS"
  exit 0
fi
...
if (( mem_available_kib < MIN_AVAILABLE_RAM_KIB )); then
  echo "error: only $((mem_available_kib / 1024)) MiB RAM is available; at least $((MIN_AVAILABLE_RAM_KIB / 1024)) MiB is required" >&2
  exit 1
fi
...
for row in "${gpu_rows[@]}"; do
  ...
  (( memory_total >= 97000 )) || {
    echo "error: GPU $index exposes only ${memory_total} MiB; 96 GB-class cards are required" >&2
    exit 1
  }
  (( memory_used <= 1024 )) || {
...
if ss -H -ltn | awk '{print $4}' | grep -Eq '(^|:)8000$'; then
  echo "error: host port 8000 is already listening; stop the active inference profile first" >&2
  exit 1
fi
...
ENVFILE="$INFERENCE_OPERATOR_HOME/.config/qwen38/flash-next-fp8-vllm-v2.env"
...
docker run -d --init \
  --restart no \
  --name "$NAME" \
  --label ai.peterstorm.inference.profile=qwen38-flash-next-fp8-vllm-v2 \
  ...
  --label ai.peterstorm.inference.qsa-topk=exact-persistent-canonical \
  --label ai.peterstorm.inference.prefix-cache=disabled \
  --gpus all \
  --ipc=host \
  --shm-size 32g \
  --network host \
...
printf "Started candidate profile '%s' with restart=no. Follow: docker logs -f %s\n" "$NAME" "$NAME"
printf "API key: %s (send as 'Authorization: Bearer <key>')\n" "$KEYFILE"
printf '%s\n' 'UVA PLE, exact persistent top-k, canonical QSA ordering, and disabled prefix caching are mandatory.'
