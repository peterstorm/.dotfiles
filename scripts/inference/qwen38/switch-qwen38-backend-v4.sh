...
# profiles. V4 adds `dflash2-vllm-official`: the digest-pinned official vLLM
# nightly containing merged DFlash2 plus its load-regression fix. Every older
# custom-image, SGLang, DSpark, and historical profile remains a rollback choice.
#
#   bash scripts/inference/qwen38/switch-qwen38-backend-v4.sh status
#   bash scripts/inference/qwen38/switch-qwen38-backend-v4.sh dflash2-vllm-official
#   bash scripts/inference/qwen38/switch-qwen38-backend-v4.sh dflash2-official
#   bash scripts/inference/qwen38/switch-qwen38-backend-v4.sh dflash2-vllm
...
ALL_NAMES=("$VLLM_V2_NAME" "$SGLANG_V2_NAME" "$DFLASH2_SGLANG_NAME" "$DFLASH2_NATIVE_NAME" "$DFLASH2_OFFICIAL_NAME" "$DFLASH2_VLLM_NAME" "$DFLASH2_VLLM_OFFICIAL_NAME" "$VLLM_V1_NAME" "$SGLANG_V1_NAME")
...
case "${1:-}" in
  status|vllm|sglang|dflash2|v1-vllm|v1-sglang|dflash2-native|dflash2-official|dflash2-vllm|dflash2-vllm-official) ;;
  *) echo "usage: $0 {status|vllm|sglang|dflash2|dflash2-native|dflash2-official|dflash2-vllm|dflash2-vllm-official|v1-vllm|v1-sglang}" >&2; exit 2 ;;
esac
MODE="${1:-status}"
...
if [ -n "${current[0]:-}" ]; then
  ...
  echo "(typically the DeepSeek-V4-Flash server: docker stop ds4-infernal-invocation-cu133-r18) — stop it explicitly first." >&2
  exit 1
fi
