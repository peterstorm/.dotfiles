...
# profiles. V5 adds `dflash2-vllm-tp1-bf16kv`: the GPU0-only vLLM profile that
# reserves GPU1 for ComfyUI. Every TP2, SGLang, DSpark, and historical profile
# remains an explicit rollback choice.
#
#   bash scripts/inference/qwen38/switch-qwen38-backend-v5.sh status
...
#   bash scripts/inference/qwen38/switch-qwen38-backend-v5.sh dflash2-vllm
#   bash scripts/inference/qwen38/switch-qwen38-backend-v5.sh sglang
#
...
DFLASH2_VLLM_OFFICIAL_NAME="qwen38-27b-bf16-dflash2-vllm-v2"
DFLASH2_VLLM_OFFICIAL_SCRIPT="$SCRIPT_DIR/run-qwen38-27b-bf16-dflash2-vllm-v2.sh"
DFLASH2_VLLM_TP1_BF16KV_NAME="qwen38-27b-bf16-dflash2-vllm-v3"
DFLASH2_VLLM_TP1_BF16KV_SCRIPT="$SCRIPT_DIR/run-qwen38-27b-bf16-dflash2-vllm-v3.sh"
ALL_NAMES=("$VLLM_V2_NAME" "$SGLANG_V2_NAME" "$DFLASH2_SGLANG_NAME" "$DFLASH2_NATIVE_NAME" "$DFLASH2_OFFICIAL_NAME" "$DFLASH2_VLLM_NAME" "$DFLASH2_VLLM_OFFICIAL_NAME" "$DFLASH2_VLLM_TP1_BF16KV_NAME" "$VLLM_V1_NAME" "$SGLANG_V1_NAME")
...
case "${1:-}" in
  status|vllm|sglang|dflash2|v1-vllm|v1-sglang|dflash2-native|dflash2-official|dflash2-vllm|dflash2-vllm-official|dflash2-vllm-tp1-bf16kv) ;;
  *) echo "usage: $0 {status|vllm|sglang|dflash2|dflash2-native|dflash2-official|dflash2-vllm|dflash2-vllm-official|dflash2-vllm-tp1-bf16kv|v1-vllm|v1-sglang}" >&2; exit 2 ;;
esac
MODE="${1:-status}"
...
case "$MODE" in
  ...
  dflash2-vllm-official) START_NAME="$DFLASH2_VLLM_OFFICIAL_NAME"; START_SCRIPT="$DFLASH2_VLLM_OFFICIAL_SCRIPT" ;;
  dflash2-vllm-tp1-bf16kv) START_NAME="$DFLASH2_VLLM_TP1_BF16KV_NAME"; START_SCRIPT="$DFLASH2_VLLM_TP1_BF16KV_SCRIPT" ;;
...
case "$MODE" in
  ...
  dflash2-vllm-official) FALLBACK="dflash2-vllm" ;;
  dflash2-vllm-tp1-bf16kv) FALLBACK="dflash2-vllm-official" ;;
...
if [ -n "${current[0]:-}" ]; then
  ...
  echo "error: port 8000 is bound but no known Qwen container is running" >&2
  echo "Another inference family owns :8000; stop that container explicitly before switching." >&2
  exit 1
fi
...
case "$START_NAME" in
  *dflash2*vllm*)
    ...
    echo "  # expected: Qwen3_5ForConditionalGeneration, then DFlash2DraftModel"
