#!/usr/bin/env bash
# Launch Qwen3.8-27B BF16 + DFlash2 on the first fixed official vLLM nightly.
#
# Versioned successor to run-qwen38-27b-bf16-dflash2-vllm.sh. The old profile
# remains the locally validated PR-head rollback. This profile uses upstream's
# digest-pinned nightly at DFlash2 load fix #53435, while retaining the measured
# local TP2/BF16 shape so the engine artifact is the principal A/B variable.
set -euo pipefail
...
CACHE_HOST="/models/vllm-cache/qwen38-bf16-dflash2-official-v2"
NAME="qwen38-27b-bf16-dflash2-vllm-v2"
...
case "$GPU_ORDER" in
  0,1|1,0) ;;
  *) echo "error: GPU_ORDER must contain both physical GPUs exactly once: 0,1 or 1,0 (got: $GPU_ORDER)" >&2; exit 2 ;;
esac
...
mapfile -t gpu_state < <(nvidia-smi --query-gpu=index,power.limit,memory.used --format=csv,noheader,nounits 2>/dev/null)
if [ "${#gpu_state[@]}" -ne 2 ]; then
  echo "error: expected two queryable GPUs before launch; found ${#gpu_state[@]}" >&2
  exit 3
fi
seen_gpus=""
for record in "${gpu_state[@]}"; do
  IFS=',' read -r gpu_index power_cap memory_used <<<"$record"
  gpu_index="${gpu_index//[[:space:]]/}"
  power_cap="${power_cap//[[:space:]]/}"
  memory_used="${memory_used//[[:space:]]/}"
  if ! [[ "$gpu_index" =~ ^[01]$ && "$power_cap" =~ ^[0-9]+([.][0-9]+)?$ && "$memory_used" =~ ^[0-9]+$ ]]; then
    echo "error: could not parse GPU state record: $record" >&2
    exit 3
  fi
  if [[ ",$seen_gpus," == *",$gpu_index,"* ]]; then
    echo "error: duplicate GPU index in nvidia-smi output: $gpu_index" >&2
    exit 3
  fi
  seen_gpus="${seen_gpus:+$seen_gpus,}$gpu_index"
  if awk -v cap="$power_cap" -v maximum="$MAX_GPU_POWER_LIMIT" 'BEGIN { exit !(cap > maximum) }'; then
    echo "error: GPU $gpu_index cap is ${power_cap}W; profile maximum is ${MAX_GPU_POWER_LIMIT}W" >&2
    exit 3
  fi
  if [ "$memory_used" -gt "$MAX_EXISTING_GPU_MEMORY_MIB" ]; then
    echo "error: GPU $gpu_index already uses ${memory_used} MiB (limit: ${MAX_EXISTING_GPU_MEMORY_MIB} MiB)" >&2
    exit 3
  fi
done
...
ENVFILE="$CONFIG_DIR/vllm-dflash2-official-v2.env"
...
docker run -d --init \
  ...
  --label io.peterstorm.inference.release="official-vllm-dflash2-v2" \
  --gpus all \
  --ipc=host \
  --network host \
  --ulimit memlock=-1 \
...
echo "Started '$NAME' with fixed official vLLM nightly $IMAGE_SOURCE_SHA."
echo "Profile: BF16 weights/KV, FP32 GDN state, TP2, DFlash2 speculative depth $DFLASH2_NUM_SPEC_TOKENS."
...
echo "Switch:   bash $SCRIPT_DIR/switch-qwen38-backend-v4.sh dflash2-vllm-official"
