#!/usr/bin/env bash
# Download and verify the exact Qwen3-TTS Base model used for direct-clone calibration.
set -euo pipefail

REPO="Qwen/Qwen3-TTS-12Hz-1.7B-Base"
REV="fd4b254389122332181a7c3db7f27e918eec64e3"
MODELS_ROOT="${VOICE_MODELS_ROOT:-/models/voice/qwen3-tts}"
DESTINATION="$MODELS_ROOT/base-1.7b"
STAGING="$MODELS_ROOT/.staging-base-$REV"
MARKER="$MODELS_ROOT/.qwen3-tts-base-complete"
LOCK="$MODELS_ROOT/.qwen3-tts-base.lock"

read -r -d '' MANIFEST <<'EOF' || true
11ad7efa24975ee4b0c3c3a38ed18737f0658a5f75a0a96787b576a78a023361 1519 .gitattributes
621b7e88f1867bc03d817176fc1f7f55f4b5a70654b2a07fdcda1e169efb024b 57817 README.md
b4f01752d15a488abde3e1ab44723ae4f4b9e68a4037257b098b3737893cc1f9 4494 config.json
...
38fc7fc51c5e776e840414b6fd443962e9411b9654888fd7913e4da643cb857c 3857413744 model.safetensors
...
main() {
  ...
  if [[ -f "$MARKER" ]] && grep -Fxq "$REPO@$REV" "$MARKER" && verify_manifest "$DESTINATION"; then
    echo "QWEN3_TTS_BASE_READY: $REPO@$REV"
    return 0
  fi
