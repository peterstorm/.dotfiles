...
| `roles/dual-desktop-plasma/default.nix` | xrandr `setupCommands` | **confirmed** `DP-6` (3840×1600 ultrawide) primary at 0,0, `DP-4` (2560×1440) to its right | Output names are per-GPU/cable — re-check with `xrandr --query` if you move a cable or swap a monitor, and flip the `--pos` values if the physical left/right is reversed |
