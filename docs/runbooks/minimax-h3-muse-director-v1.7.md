# MiniMax H3 Muse Director v1.7 — maximum-quality local profile
...
```text
/var/lib/comfyui/user/default/workflows/minimax-h3-muse-director-v12-local-development-v1.7/
└── 01 MiniMax H3 Muse Director V1.7 - Maximum Quality BF16 Local Development.json
```

V1.5 and V1.6 remain untouched. Every future graph change must use a new filename and destination directory.
...
The Analyze backend is pinned to the authenticated local vision endpoint and its currently served model ID: `glm-5.3-flash-exl3-k4-vision-fp8kv-mtp-359k-v11.1`. V1.7 fails its graph contract if that serialized ID drifts.
...
# Active H3 sampling preview. Requires the MiniMax license and territorial
# authorization gates.
...
The video-linked LTX2.3 preview VAE may remain installed, but V1.7 does not select it: it appeared only in an unlinked, bypassed fallback loader. Its active and named-widget selectors are `(disabled)`. The two active KJNodes preview overrides use `vae_approx/taeh3.safetensors` instead.
...
3. Open the v1.7 workflow and verify the Unified Loader displays `Maximum Quality`.
