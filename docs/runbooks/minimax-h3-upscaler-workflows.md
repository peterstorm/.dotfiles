...
The exact Veteran AI R2V graph and its active selectors are preserved in `development/research/minimax-h3-video-editing-workflow-05/`. Its 10Eros Ref2VA checkpoint, NVFP4 Qwen encoder, and learned latent-upscaler weight are checksum-installed. Treat every result as an unqualified Development derivative pending identity, topology, temporal-stability, audio-latent, and refinement benchmarks.

The desktop closure also pins `ComfyUI-MMH3Tools` v0.87.0 at `7194baedb1cfeb6b55d0797a3af849b73a69c6c2`, the first revision containing every MMH3 node used by the latest context-window graph, including `MMH3ImageList`. That revision expects the newer private `_encode_ref_audio` H3 helper, so the Nix closure supplies its small pure implementation when running pinned ComfyUI 0.33.3. Current MMH3Tools HEAD adds more incompatible private-helper dependencies and is not used. MMH3Tools' `pyproject.toml` declares MIT. The installed local v3 graph uses MMH3Tools for context windows but replaces the upstream ClownScheduler with core BasicScheduler and the RTX VFX resize with core Lanczos, avoiding RES4LYF and Windows-only runtime dependencies.

The older `minimax-h3-upscaler-research-only` browser folder remains removed because its unchanged third-party examples select missing quantized/pruned H3 models and incompatible Turbo LoRAs. Checksum-identical originals remain preserved as private Development evidence in `development/research/minimax-h3-encyclopedia-01/workflows/h3-latent-upscaler/`; they are not executable authorities.
